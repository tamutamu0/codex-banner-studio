...
export function missingRequiredVariables(step: PromptStep, template: string) {
  return REQUIRED_VARIABLES[step].filter((key) => !template.includes(`{{${key}}}`));
}
...
function normalizeCustomPreset(item: Partial<PromptPreset>): PromptPreset | null {
  ...
  return {
    id: item.id && !String(item.id).startsWith("default-") ? String(item.id) : makeId("prompt"),
    step,
    name,
    template,
    builtIn: false,
...
export async function readPromptPresets() {
  ...
  try {
    ...
    custom = Array.isArray(parsed) ? parsed.map(normalizeCustomPreset).filter(Boolean) as PromptPreset[] : [];
  } catch {
    custom = [];
  }
  return [...DEFAULT_PROMPT_PRESETS, ...custom];
}

export async function saveCustomPromptPresets(presets: PromptPreset[]) {
  await ensureMasterDirs();
  const custom = presets.filter((item) => !item.builtIn).map(normalizeCustomPreset).filter(Boolean) as PromptPreset[];
  await writeFile(presetPath, JSON.stringify(custom, null, 2), "utf8");
  return [...DEFAULT_PROMPT_PRESETS, ...custom];
}
