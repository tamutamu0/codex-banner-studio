...
export async function POST(request: Request) {
  ...
  const { input, variants, divisions = 4, sheetRuns = 1, cancelKey, codexSettings, promptTemplates } = (await request.json()) as Body;
  ...
  const productNotes = input.notes || "なし。商品やカテゴリを調べて、強い広告表現を自動で考える。";
  const sheetBlocks = Array.from({ length: runCount }, (_, index) => sheetBlock(variants, input, count, index + 1)).join("\n\n");
  const defaultPrompt = `
